...
const JwtAuthCallback = async (jwt_payload, done) => {
  try {
    let user = await User.findById(jwt_payload.sub).select(
...
const FacebookAuthCallback = async (
  ...
  try {
    ...
    if (existingUser) {
      ...
      console.log("User already exist with same email");
      console.log("LINK ACCOUNT");
      existingUser.provider = ["local", "facebook"];
  ...
  new FacebookStrategy(
    {
      ...
      clientSecret: process.env.FB_CLIENT_SECRECT,
